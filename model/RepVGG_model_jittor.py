...
class RepVGG_Model(nn.Module):
    ...
    def __init__(self, channel_scale_A, channel_scale_B, group_conv, classify_classes, model_type):
        ...
        for stage_id in range(5):
            ...
            if stage_id == 0:
                channel_size = min(channel_size, 64)
            if stage_id == 4:
                channel_size = int(channel_scale_B * channel_scale_base[stage_id])

            RepVGG_Blcoks = RepVGG_Blcoks + self.get_a_stage(channel_size, blocks_per_stage[stage_id])
